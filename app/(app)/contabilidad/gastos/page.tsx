...
export default async function ExpensesPage({
  ...
  const monthHref = (target: string) =>
    `/contabilidad/gastos?m=${target}${cat ? `&cat=${cat}` : ""}`;
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            ...
            {manager && (
              <Link href="/contabilidad/finanzas" className="font-medium text-primary hover:underline">
                Ver balance mensual →
              </Link>
            )}
          </p>
      ...
      <div className="flex flex-wrap gap-1.5">
        <Link
          href={`/contabilidad/gastos?m=${month}`}
        ...
        {categories
          ...
          .map((c) => (
            <Link
              key={c.id}
              href={`/contabilidad/gastos?m=${month}&cat=${c.id}`}
